...
{{/* Vars */}}
{{$isMod := false}}{{$map := ""}}{{$error := ""}}{{$day := 0}}{{$month := 0}}{{$year := 0}}{{$isUnderAge := false}}{{$isValidDate := false}}{{$user := .User}}{{$checkDate := ""}}{{$insideMap := sdict}}{{$list := cslice}}{{$out := ""}}{{$send := false}}{{$userMonth := ""}}{{$today := sdict}}
...
{{if .ExecData}}
	...
	{{range (index $today (str currentTime.Day))}}
		...
		{{end}}
	{{end}}
	{{if $send}} {{sendMessageNoEscape nil $bdayMsg}} {{end}}
{{else}}
	{{if $isMod}}
